...
def build_deep_value():
    ...
    for slug, abs_json_str in tqdm(data, desc="Parsing Deep Value"):
        try:
            ...
            pe = d.get('peRatio', np.nan)
            pb = d.get('pbRatio', np.nan)
            peg = d.get('pegRatio', np.nan)
            ...
            if ohlcv:
                ...
                if len(df) > 0 and 'Close' in df.columns:
                    ...
                    days_since_trade = (pd.Timestamp('2026-06-25') - latest_date).days
                    ...
                    if high_52w > 0:
                        discount_from_high = (high_52w - latest_close) / high_52w
            ...
            row = {
                ...
                'discount_from_high': discount_from_high
            }
            rows.append(row)
    ...
    for col in ['peRatio', 'pbRatio', 'pegRatio', 'roe']:
        df[col] = pd.to_numeric(df[col], errors='coerce')
        
    os.makedirs('datasets/active/deep_value', exist_ok=True)
    df.to_parquet('datasets/active/deep_value/deep_value_data.parquet', index=False)
