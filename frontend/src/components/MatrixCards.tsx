            ...
            <div className="flex-1 bg-surface/30 rounded p-1.5 border border-border/50 flex flex-col justify-center relative">
              ...
              {factor_exposure?.momentum_score === "ERR_DATA_MISSING" ? (
                <div className="flex flex-col">
                  <span className="font-mono text-red-500 font-bold text-[10px] bg-red-500/10 py-0.5 px-1 rounded w-fit">ERR_DATA_MISSING</span>
                  <span className="text-[7px] text-text-secondary mt-0.5 truncate">rs_rating missing from parquet</span>
                </div>
              ) : (
      ...
      <>
        {renderCard("Ensemble Alpha", <BrainCircuit size={12} className="text-indigo-400" />, (
          <div className="p-2 bg-surface/50 rounded border border-border text-center flex flex-col h-full justify-center">
             ...
             <span className={`text-3xl font-bold font-mono ${ensemble_alpha > 50 && ensemble_alpha !== "PENDING" ? 'text-alpha' : 'text-beta'}`}>{ensemble_alpha === "PENDING" ? 0 : ensemble_alpha || 0}%</span>
          </div>
        ))}
        {renderCard("Forensic Auditor", <AlertTriangle size={12} className="text-indigo-400" />, (
          <div className="p-2 bg-surface/50 rounded border border-border text-center flex flex-col h-full justify-center">
             ...
             <span className={`text-3xl font-bold font-mono ${forensic_risk > 30 && forensic_risk !== "PENDING" ? 'text-beta' : 'text-alpha'}`}>{forensic_risk === "PENDING" ? 0 : forensic_risk || 0}%</span>
          </div>
        ))}
