...
export const Screener: React.FC = () => {
  ...
  const fetchResults = useCallback(async () => {
    ...
    setError(null);
    try {
      const endpoint = `http://127.0.0.1:8000/api/screener/${mode === 'stocks' ? 'stocks' : 'mutual-funds'}`;
  ...
  return (
    <div className="flex flex-col h-full bg-[#09090c] text-white overflow-hidden">
      ...
      {/* ── QUERY BUILDER AREA ───────────────────────────────────────────── */}
      <div className="px-6 py-4 border-b border-white/[0.06] bg-[#111116] flex flex-col gap-3">
         ...
         {/* Category quick buttons (renders as an extra ribbon under the builder) */}
         <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] text-white/30 uppercase font-bold tracking-widest mr-2">Categories</span>
                  ...
                  <button
                     ...
                     className={`px-3 py-1 text-[10px] uppercase tracking-wider font-bold rounded border transition-all ${
                        filterPanelOpen === group
                           ? 'bg-alpha/10 border-alpha/30 text-white'
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-white/5 mt-1">
               {allMetrics.filter(m => m.group === filterPanelOpen).map(m => (
                  <button
                     ...
                     className="px-2 py-1 bg-[#1a1a24] hover:bg-alpha/20 border border-white/5 hover:border-alpha/30 rounded text-xs text-white/70 hover:text-white transition-colors"
                  >
