...
import { Shield, TrendingUp, TrendingDown, Clock, Activity, HelpCircle } from 'lucide-react';
...
export const MacroResilience = ({ data }: { data: any }) => {
  ...
  return (
    <div className="bg-surface p-5 rounded-xl border border-border h-full flex flex-col col-span-1 relative overflow-hidden">
      {/* Decorative gradient */}
        ...
        <div>
          <h3 className="text-sm font-semibold text-text-primary flex items-center gap-1.5 shrink-0 group relative w-fit cursor-help">
            ...
            <div className="absolute bottom-full left-0 mb-2 hidden group-hover:block w-64 bg-surface-hover text-text-primary text-[10px] p-2 rounded shadow-xl z-50 normal-case tracking-normal border border-border font-normal leading-relaxed">
              Evaluates the stock's defensive characteristics and reaction to broader Nifty 50 volatility and VIX spikes.
            </div>
          </h3>
          <p className="text-[10px] text-text-secondary mt-1 uppercase tracking-wider font-bold">Calculated over a 3-year window</p>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4">
                {/* Up Capture */}
                <div className="bg-canvas p-3 rounded-lg border border-border flex flex-col gap-2">
        ...
        {/* Stress Metrics */}
        <div className="bg-canvas rounded-lg border border-border p-3 flex flex-col gap-3">
            <div className="flex justify-between items-center">
